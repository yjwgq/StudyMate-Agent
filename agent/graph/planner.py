...
1. 只有**纯问答**（解释概念、总结资料、闲聊，用知识库或常识即可回答，不需要执行任何动作）→
   {"mode": "chat", "steps": []}

2. 只要请求需要**执行动作**或调用工具，就必须用 plan（不要用 chat 回避）：
   发邮件/消息、创建或修改待办、运行代码计算、联网搜索、多方面调研、对比多个对象 →
   例：「帮我发邮件给 X」「加一条待办」「算一下 1234×5678」「调研 A 和 B 的区别」全部属于此类。
   {"mode": "plan", "steps": [
